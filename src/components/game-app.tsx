  ...
  if (!user) {
    return (
      <Link
        to="/login"
        data-ui
        className="inline-flex h-10 items-center rounded-full border border-border bg-surface px-3 text-xs font-medium text-muted"
      >
        Sign in
      </Link>
    );
    ...
    <div className="flex h-dvh w-full flex-col overflow-hidden bg-bg text-fg">
      <div className="relative min-h-0 flex-1">
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 px-3 pt-[max(0.5rem,env(safe-area-inset-top))] sm:pt-3 [text-shadow:0_1px_10px_rgba(20,28,16,0.7)]">
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-xs text-muted">
                {playing ? dog.name : "Best"}
                {hud.combo >= 2 && playing ? (
                  <span className="ml-2 text-accent tabular-nums">x{hud.combo}</span>
                ) : null}
                {playing && hud.threads > 0 ? (
                  <span className="ml-2 text-accent tabular-nums">{hud.threads} hoops</span>
                ) : null}
              </p>
            </div>
      ...
      {overlay === "title" || overlay === "boot" ? (
        <div
          data-ui
          className="shrink-0 border-t border-border bg-surface px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
        >
          <p className="text-center text-[10px] tracking-[0.32em] text-muted uppercase">Park pack</p>
          <h1 className="font-display text-center text-3xl leading-none font-extrabold tracking-tight">Packline</h1>
          <p className="mt-1 text-center text-sm text-muted">Tap a dog to run the park.</p>
            ...
            {DOG_IDS.map((id) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "min-h-24 rounded-lg border bg-bg px-1 py-2",
          ...
          <p className="mt-2 text-center text-xs text-subtle">Tap to jump · Hold slide · Teddy walks tunnels</p>
        </div>
      ) : null}
      ...
      {overlay === "gameover" ? (
        <div
          data-ui
          className="shrink-0 border-t border-border bg-surface px-4 pt-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
        >
          ...
          <dl className="mt-3 grid grid-cols-4 gap-2 text-sm">
            ...
            <div className="rounded-md bg-elevated px-2 py-2">
              <dt className="text-[10px] text-muted">Best</dt>
              <dd className="font-mono text-xs tabular-nums">{formatScore(hud.highScore)}</dd>
            </div>
            <div className="rounded-md bg-elevated px-2 py-2">
      ...
      {playing ? (
        <div
          data-ui
          className="grid shrink-0 grid-cols-[1fr_auto] gap-3 bg-bg px-3 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
        >
          <p className="self-center text-xs text-muted">Tap the run to jump</p>
          <button
            type="button"
            data-ui
            aria-label="Slide"
            className="flex h-16 min-w-28 items-center justify-center gap-2 rounded-lg bg-accent text-base font-semibold text-accent-fg"
            {...slideHold}
          >
